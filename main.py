...
def print_id_dataset_table(id_dataset):
    ...
    for i in range(nrof_samples):
        ...
        for j in range(nrof_samples):
            dist = get_embedding_distance(
                id_dataset[i].embedding, id_dataset[j].embedding)
            print('  %1.4f      ' % dist, end='')
        print('')
...
def test_run(pnet, rnet, onet, sess, images_placeholder, phase_train_placeholder, embeddings, id_dataset, test_folder):
    ...
    image_names = [f for f in os.listdir(test_folder) if not f.startswith('.')]
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            id_dataset = id_data.get_id_data(
                args.id_folder[0], pnet, rnet, onet, sess, embeddings, images_placeholder, phase_train_placeholder)
...
def parse_arguments(argv):
    ...
    parser.add_argument(
        'model', type=str, help='Could be either a directory containing the meta_file and ckpt_file or a model protobuf (.pb) file')
